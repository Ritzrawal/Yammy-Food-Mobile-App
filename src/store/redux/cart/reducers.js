...
export const cart = (state = initialState, action) => {
  switch (action.type) {
    ...
    case IMCartActionsConstants.UPDATE_PRICE: {
...
export const storeCartToDisk = async (cartItems, vendor) => {
  ...
  // Store on disk
  localStorage.setItem('@MySuperCart:key', serializedCart);
};
...
export const removeFromCart = (item, state) => {
  ...
  localStorage.setItem('@MySuperCart:key', JSON.stringify(cartArray));
